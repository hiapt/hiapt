...
<table class="table beauty-table table-hover" style="text-align: center;">
					<thead>
						...
						<td><%= m.getEmpEmail() %></td>
						<td><a href="/hiapt/mdetail?mailno=<%= m.getMailNo() %>&page=<%= currentPage %>"><%= m.getMailTitle() %></a></td> <%-- &allmail=<%= allmail %> --%>  
						<%-- &code=<%= code %> --%>
						<td><%= m.getMailTime() %></td>
						</tr>
						<% } %>
			...
			<div class="paging_simple_numbers">
				<ul class="pagination" style="justify-content: center;">
					<li class="paginate_button page-item previous disabled"
						id="dataTable_previous"><a href="/hiapt/mlist?page=1&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable"
					...
					<li class="paginate_button page-item active back"
						id="dataTable_previous"><a href="/hiapt/mlist?page=<%= beginPage -10 %>&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable"
						data-dt-idx="<%= beginPage - 10 %>" tabindex="0" class="page-link">&lsaquo;&lsaquo;</a></li>
					<% } %>
					<% if(endPage == 0){ %>
					<li class="paginate_button page-item active"
						id="dataTable_previous"><a href="/hiapt/selfmlist?page=1&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable"
						data-dt-idx="0" tabindex="0" class="page-link">1</a></li>
					<% }else{ %>
					<% for(int p = beginPage; p <= endPage; p++){
						if(p == currentPage){	
					%>
					<li class="paginate_button page-item active next"><a href="/hiapt/mlist?page<%= p %>&empemail=<%= emp.getEmpEmail() %>&mcode=1"
						aria-controls="dataTable" data-dt-idx="<%= p %>" tabindex="0"
						class="page-link"><%= p %></a></li>
					<% } else { %>
					<li class="paginate_button page-item next" id="dataTable_next"><a
						href="/hiapt/mlist?page=<%= p %>&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable" data-dt-idx="<%= p %>" tabindex="0"
						class="page-link"><%= p %></a></li>
					<% }} %>
					<% } %>
					<% if((endPage + 10) > maxPage ) { %>
					<li class="paginate_button page-item next" id="dataTable_next"><a
						href="/hiapt/mlist?page=<%= maxPage %>&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable" data-dt-idx="<%= maxPage %>" tabindex="0"
						class="page-link">&rsaquo;&rsaquo;</a></li>
					<% } else { %>	
					<li class="paginate_button page-item next" id="dataTable_next"><a
						href="/hiapt/mlist?page<%= endPage + 10 %>&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable" data-dt-idx="<%= endPage + 10 %>" tabindex="0"
						class="page-link">&rsaquo;&rsaquo;</a></li>
					<% } %>
					<li class="paginate_button page-item next" id="dataTable_next"><a
						href="/hiapt/mlist?page=<%= maxPage %>&empemail=<%= emp.getEmpEmail() %>&mcode=1" aria-controls="dataTable" data-dt-idx="<%= maxPage %>" tabindex="0"
