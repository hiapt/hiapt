...
/**
 * Servlet implementation class MyboxEmpServlet
 */
@WebServlet("/mylist.emp")
public class MyboxEmpServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MyboxEmpServlet() {
        super();
	...
	String myboxcode = request.getParameter("myboxcode");
		...
		int listCount = mservice.getMyboxCount(empno, myboxcode);
		//총 페이지 수 계산
		...
		ArrayList<Mybox> list = mservice.selectList(startRow, endRow, empno, myboxcode);
		RequestDispatcher view = null; //아래 if else 둘 다에서 사용함 --> 미리 만들어놓음

		
		if(list.size() > 0) {
			
			view = request.getRequestDispatcher("views/emp/approval/empMybox.jsp");
