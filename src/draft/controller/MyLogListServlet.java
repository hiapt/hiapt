...
/**
 * Servlet implementation class MyLogListServlet
 */
@WebServlet("/llist.my")
public class MyLogListServlet extends HttpServlet {
	private static final long serialVersionUID = 1L;
       
    /**
     * @see HttpServlet#HttpServlet()
     */
    public MyLogListServlet() {
        super();
	...
	String empno = request.getParameter("empno");
		...
		int listCount = dservice.getMyLogCount(empno);  //테이블의 전체 목록 갯수 조회
		//총 페이지 수 계산
		...
		ArrayList<Draft> list = dservice.selectLogMy(startRow, endRow, empno);
		RequestDispatcher view = null; //아래 if else 둘 다에서 사용함 --> 미리 만들어놓음
	
		
		if(list.size() > 0) {
			
			view = request.getRequestDispatcher("views/emp/approval/myLogList.jsp");
